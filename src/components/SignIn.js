...
const SignIn = () => {
	...
	return (
		...
		<div className='w-2/3  mx-auto '>
		 <h1 className='text-white text-xl font-semibold'><img src="./LogoDashMasjid.png" alt="" className='inline-block w-[50px]' />DASHMASJID</h1>
		 <div className='justify-center text-white  mt-40'>
         
            <h1 className='text-3xl font-bold'>Welcome, to DashMasjid</h1>
			<h1 className='my-2 font-bold mb-10 mt-3'>Please enter your details</h1>

			<label htmlFor="email" className='text-[10px] font-bold'>EMAIL</label>
			<input type="email" name="email" onChange={changeemail} value={email} placeholder="Email Adresss" className='text-black block my-3 sm:w-full lg:w-1/2 rounded-3xl px-3 py-2 bg-gray-200 border-none' id="" />
			<label htmlFor="email" className='text-[10px] font-bold'>PASSWORD</label>
			<input type="password" name="password" onChange={changepassword} value={password} placeholder="Password" className='text-black block my-3 sm:w-full lg:w-1/2 rounded-3xl px-3 py-2 bg-gray-200 border-none' id="" />
			<button onClick={signup} className="lg:w-1/2 sm:w-full my-4 py-2 bg-black backdrop-blur-2xl bg-opacity-30 border-[1px] rounded-xl">Sign in</button>
			<h2 className='text-gray-300 text-center lg:w-1/2 sm:w-full'>Don't have an account? <Link className='text-white font-bold' to={'/signup'}>Sign up for free</Link></h2>
		 </div> 
		</div>
