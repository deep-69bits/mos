      ...
      onAuthStateChanged(auth, (user) => {
          if(user){
             const retdata=async()=>{
              ...
              data.forEach((doc) => {
                setCommitiedata(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
              });
              setLoading(false);
             }
             retdata( )
        ...
        <div className="grid grid-flow-row grid-cols-2 gap-x-20 px-40 py-40 ">

          {
            commitiedata.map((item, index) => {
              return (
                item.type == "member" ?
                  <div className="bg-white text-[#E1C49A] transition-all duration-500 hover:scale-105 rounded-full text-center py-5">
                     <h1 className="text-3xl font-semibold">{item.name}</h1>
                     <h2 className="py-1">{item.position}</h2>
                     <button className=" w-[200px] py-2 bg-black border-none  text-[#E1C49A]  rounded-2xl">
